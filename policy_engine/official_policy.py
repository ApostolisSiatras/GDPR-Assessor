...
from .storage import hash_text, write_secure_text
...
def _assemble_official_policy(context: Dict[str, Any], sections: List[Dict[str, str]]) -> str:
    ...
    today = datetime.now(UTC).date().isoformat()
    lines = [
        f"# Official GDPR Compliance Policy", f"Last updated: {today}", "", f"This policy applies to **{org_name}**.", "",
    ]
...
def load_official_policy(run_id: str | None) -> Dict[str, Any] | None:
    ...
    metadata["markdown"] = policy_path.read_text(encoding="utf-8")
